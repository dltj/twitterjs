 ...
 * $Date$
 * Modified by Peter Murray: http://github.com/dltj/twitterjs
 */
 /*
  MIT (MIT-LICENSE.txt)
 */
if(typeof renderTwitters!='function')(function(){var browser=(function(){var b=navigator.userAgent.toLowerCase();return{webkit:/(webkit|khtml)/.test(b),opera:/opera/.test(b),msie:/msie/.test(b)&&!(/opera/).test(b),mozilla:/mozilla/.test(b)&&!(/(compatible|webkit)/).test(b)};})();var guid=0;var readyList=[];var isReady=false;var monthDict=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];var statusCookieName="statusCookie";window.ify=function(){var entities={'"':'"','&':'&','<':'<','>':'>'};return{"link":function(t){return t.replace(/[a-z]+:\/\/[a-z0-9-_]+\.[a-z0-9-_:~%&\?#\/.=]+[^:\.,\)\s*$]/ig,function(m){return'<a href="'+m+'">'+((m.length>25)?m.substr(0,24)+'...':m)+'</a>';});},"at":function(t){return t.replace(/(^|[^\w]+)\@([a-zA-Z0-9_]{1,15}(\/[a-zA-Z0-9-_]+)*)/g,function(m,m1,m2){return m1+'@<a href="http://twitter.com/'+m2+'">'+m2+'</a>';});},"hash":function(t){return t.replace(/(^|[^&\w'"]+)\#([a-zA-Z0-9_]+)/g,function(m,m1,m2){return m1+'#<a href="http://search.twitter.com/search?q=%23'+m2+'">'+m2+'</a>';});},"clean":function(tweet){return this.hash(this.at(this.link(tweet)));}};}();window.renderTwitters=function(obj,options){function node(e){return document.createElement(e);}
function text(t){return document.createTextNode(t);}
var target=document.getElementById(options.twitterTarget);var data=null;var ul=node('ul'),li,statusSpan,timeSpan,i,max=obj.length>options.count?options.count:obj.length;for(i=0;i<max&&obj[i];i++){data=getTwitterData(obj[i]);if(options.ignoreOlderThan>0){var parsed_date=parse_twitter_date(obj[i].created_at),delta=calc_relative_time(parsed_date);if(delta>options.ignoreOlderThan){break;}}
if(options.stopIfSeen&&obj[i].text.match(options.stopIfSeen)){break;}
...
ul.appendChild(li);}
if(ul.hasChildNodes()){if(options.clearContents){while(target.firstChild){target.removeChild(target.firstChild);}}
target.appendChild(ul);}
setTweetCookie(options.statusCookieName,target.innerHTML,options.cookieDomain,options.cookieRefresh);if(typeof options.callback=='function'){options.callback();}};window.getTwitters=function(target,id,count,options){guid++;if(typeof id=='object'){options=id;id=options.id;count=options.count;}
...
if(typeof options.ignoreOlderThan=='undefined'){options.ignoreOlderThan=24*60*60;}
if(options.withFriends)options.withFriends=false;options['twitterTarget']=target;options['statusCookieName']=(options['cokiePrefix']?options['cookiePrefix']:"")+target;if(typeof options.enableLinks=='undefined')options.enableLinks=true;window['twitterCallback'+guid]=function(obj){if(options.timeout){clearTimeout(window['twitterTimeout'+guid]);}
...
var cachedTweets=getTweetCookie(options.statusCookieName);if(cachedTweets){document.getElementById(options.twitterTarget).innerHTML=cachedTweets;return;}
...
var script=document.createElement('script');script.setAttribute('src',url);document.getElementsByTagName('head')[0].appendChild(script);};})(options,guid));};DOMReady();function setTweetCookie(statusCookieName,tweetHtml,cookieDomain,cookieRefresh){if(cookieDomain&&cookieRefresh>0){var today=new Date();today.setTime(today.getTime());var expires=cookieRefresh*1000*60;var expires_date=new Date(today.getTime()+(expires));document.cookie=statusCookieName+"="+escape(tweetHtml)+";expires="+expires_date.toGMTString()+";path=/"+";domain="+cookieDomain;}
return;}
function getTweetCookie(statusCookieName){var results=document.cookie.match('(^|;) ?'+statusCookieName+'?=([^;]*)(;|$)');if(results){return(unescape(results[2]));}else{return null;}}
...
function parse_twitter_date(time_value){var values=time_value.split(" "),parsed_date=Date.parse(values[1]+" "+values[2]+", "+values[5]+" "+values[3]);return parsed_date;}
function calc_relative_time(parsed_date){var relative_to=(arguments.length>1)?arguments[1]:new Date(),delta=parseInt((relative_to.getTime()-parsed_date)/1000);;delta=delta+(relative_to.getTimezoneOffset()*60);return delta;}
function relative_time(time_value){function formatTime(date){var hour=date.getHours(),min=date.getMinutes()+"",ampm='AM';if(hour==0){hour=12;}else if(hour==12){ampm='PM';}else if(hour>12){hour-=12;ampm='PM';}
...
var parsed_date=parse_twitter_date(time_value),date=new Date(parsed_date),delta=calc_relative_time(parsed_date),r='';if(delta<5){r='less than 5 seconds ago';}else if(delta<30){r='half a minute ago';}else if(delta<60){r='less than a minute ago';}else if(delta<120){r='1 minute ago';}else if(delta<(45*60)){r=(parseInt(delta/60)).toString()+' minutes ago';}else if(delta<(2*90*60)){r='about 1 hour ago';}else if(delta<(24*60*60)){r='about '+(parseInt(delta/3600)).toString()+' hours ago';}else{if(delta<(48*60*60)){r=formatTime(date)+' yesterday';}else{r=formatTime(date)+' '+formatDate(date);}}
return r;}})();getTwitters('ohiolinkstatus',{id:'OhioLINKstatus',count:20,ignoreReplies:true,ignoreOlderThan:36*60*60,stopIfSeen:'returned to normal',cookiePrefix:'TwitterJS',cookieDomain:'dltj.org',cookieRefresh:5});